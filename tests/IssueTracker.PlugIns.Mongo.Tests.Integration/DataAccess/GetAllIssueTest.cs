...
public class GetAllIssueTest : IAsyncLifetime
{
	...
	public GetAllIssueTest(IssueTrackerTestFactory factory)
	{
	...
	public async Task GetAllAsync_With_Valid_Data_Should_Succeed_TestAsync(int count, bool includeArchived, int expected)
	{
		...
		// Arrange
		List<IssueModel> categories = FakeIssue.GetIssues(count).ToList();
		...
		// Act
		List<IssueModel> result = (await _sut.GetAllAsync(includeArchived).ConfigureAwait(false))!.ToList();
