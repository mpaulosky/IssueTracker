...
public class GetAllCategoryTest : IAsyncLifetime
{
	...
	public async Task GetAllAsync_With_Valid_Data_Should_Succeed_TestAsync(int count, bool includeArchived, int expected)
	{
		...
		// Arrange
		List<CategoryModel> categories = FakeCategory.GetCategories(count).ToList();
		...
		// Act
		List<CategoryModel> result = (await _sut.GetAllAsync(includeArchived).ConfigureAwait(false))!.ToList();
